...
require 'byebug'
class Course
  attr_accessor :name, :credits, :students, :department, :days, :time_block
  ...
  def conflicts_with?(course)
    days_in_str = course.days.map {|day| day.to_s}
    self.days.each do |day|
      if days_in_str.include?(day.to_s) && self.time_block == course.time_block
        return true
      end
    end
    false
